...
class Plugin extends AbstractPlugin
{
    ...
    public function attachSubscribeNotice($subscribe)
    {
        ...
        $notice = [
            ...
            'limits' => [
                ...
                'source_batch_user_threshold' => (int) $this->getConfig('source_batch_user_threshold', 3),
                'source_batch_window_seconds' => (int) $this->getConfig('source_batch_window_seconds', 600),
    ...
    private function appendRecentEvent(?int $userId, ?string $email, string $code, string $reason, array $meta = [], array $notificationResult = []): void
    {
        ...
        $event = [
            ...
            'online_ip_count' => isset($meta['online_ip_count']) ? (int) $meta['online_ip_count'] : null,
            'source_user_count' => isset($meta['source_user_count']) ? (int) $meta['source_user_count'] : null,
            'source_user_threshold' => isset($meta['source_user_threshold']) ? (int) $meta['source_user_threshold'] : null,
