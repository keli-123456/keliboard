...
4. **IP限制** - 限制时间窗口内不同IP的访问数量
5. **访问频率限制** - 限制用户的订阅拉取频率  
6. **阅后即焚** - 限制订阅链接在指定时间内的使用次数
...
2. **在线唯一IP阈值** → 检查当前在线唯一IP数是否超限
3. **IP限制** → 检查时间窗口内IP数量是否超限
4. **频率限制** → 检查时间窗口内访问次数是否超限
5. **阅后即焚** → 检查时间窗口内使用次数是否超限
